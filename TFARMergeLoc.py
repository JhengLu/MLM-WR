...
def TFARMergeLocF(thisround,fileId):

    RelLocOrder = None
    workerNumber = parameter.workerNumber
    allWorkerList = list(range(1,1+workerNumber))
    InteLocOrder = pd.DataFrame()
    locationNumber = parameter.locationNumber
    if thisround == 2:  # 因为只有第一轮有了
        RelLocOrderFile = "file_" + str(fileId) + "/" + "TFARworkerOrder/workerRelativeLoc_" + str(thisround - 1) + ".csv"
        RelLocOrder = pd.read_csv(RelLocOrderFile)
        locationName = list(RelLocOrder.columns)
        for name in locationName:
            thisOrder = RelLocOrder[name].tolist()
            zeroIndex = thisOrder.index(0)
            thisOrder = thisOrder[:zeroIndex]
            secondPart = list(set(allWorkerList) - set(thisOrder))
            random.shuffle(secondPart)
            InteLocOrder[name] = thisOrder + secondPart

    else:
        RelLocOrder = pd.DataFrame()
        lastround = thisround - 1
        thisRelLocOrderFile = "file_" + str(fileId) + "/" + "TFARworkerOrder/workerRelativeLoc_" + str(thisround - 1) + ".csv"  # this指的是本轮次上一轮
        thisRelLocOrder = pd.read_csv(thisRelLocOrderFile)

        lastRelLocOrderFile = "file_" + str(fileId) + "/" + "TFARworkerOrder/workerRelativeLoc_" + str(lastround - 1) + ".csv"  #指的是本轮次的上一轮的上一轮
                    ...
                    for i in range(len(thisOrder) - 1):
                        if thisOrder[i] in common_list and thisOrder[i + 1] in common_list:
                            ...
                            if index2 > index1 + 1:
                                ...
                                part = [x for x in part if x not in thisOrder]
    ...
    InteLocOrderFile = "file_" + str(fileId) + "/" + "TFARworkerOrder/integLoc_" + str(thisround - 1) + ".csv"
    InteLocOrder.to_csv(InteLocOrderFile, index=False)
